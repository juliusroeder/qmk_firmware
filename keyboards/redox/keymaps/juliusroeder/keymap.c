...
bool dip_switch_update_user(uint8_t index, bool active) { 
    switch (index) {
        case 0:
            if(active) { register_code(KC_MEDIA_PLAY_PAUSE);} else { unregister_code(KC_MEDIA_PLAY_PAUSE);}
            break;
        case 1:
            // if(active) { layer_on(_FUNC); } else { layer_off(_FUNC); }
            if(active) {register_code(KC_AUDIO_MUTE);} else {unregister_code(KC_AUDIO_MUTE);}
            break;
    }
    return true;
}
